...
def test_duplicate_username_returns_400():
    client.post("/api/v1/auth/register", json={"username": "alice", "password": "pass123"})
    response = client.post("/api/v1/auth/register", json={"username": "alice", "password": "pass123"})
...
def test_register_login_then_access_me():
    # Step 1: register
    client.post("/api/v1/auth/register", json={"username": "bob", "password": "secret99"})
    
    # Step 2: login
    login_res = client.post("/api/v1/auth/login", json={"username": "bob", "password": "secret99"})
